...
#define OpenUDIXXLog(fmt, ...)
//#define OpenUDIXXLog(fmt, ...) XXLog((@"%s [Line %d] " fmt), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
//#define OpenUDIXXLog(fmt, ...) XXLog((@"[Line %d] " fmt), __LINE__, ##__VA_ARGS__);
...
+ (NSMutableDictionary*) _getDictFromPasteboard:(id)pboard {
    ...
    if (item) {
        @try{
            ...
            OpenUDIXXLog(@"Unable to unarchive item %@ on pasteboard!", [pboard name]);
            item = nil;
        }
    }
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    if ([localDict isKindOfClass:[NSDictionary class]]) {
        ...
        OpenUDIXXLog(@"localDict = %@",localDict);
    }
...
#endif
        OpenUDIXXLog(@"SlotPB name = %@",slotPBid);
        if (slotPB==nil) {
    ...
    //
    OpenUDIXXLog(@"Available Slot %@ Existing Slot %@",availableSlotPBid,myRedundancySlotPBid);
...
+ (void) setOptOut:(BOOL)optOutValue {
    ...
    OpenUDIXXLog(@"Local dict after opt-out = %@",dict);
