...
- (void)loadNextView
{
    ...
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(cardContainerViewNumberOfViewInIndex:)]) {
        ...
        XXLog(@"index = %zd, _loadedIndex = %zd, _currentIndex = %zd", index, _loadedIndex, _currentIndex);
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture
{
    ...
    if (gesture.state == UIGestureRecognizerStateChanged) { // 2
        ...
        XXLog(@"point.y  =%f, gesture.view.center.y = %f, movedPoint.y = %f", point.y, gesture.view.center.y, movedPoint.y);
        ...
        XXLog(@"scale = %f", (gesture.view.center.y - _cardCenterY) / _cardCenterY);
        XXLog(@"make = %f", (gesture.view.center.y - _cardCenterY) / _cardCenterY * (_moveSlope * (M_PI / 20)));
...
//        XXLog(@"前gesture.x = %f, gesture.y = %f", prePoint.x, prePoint.y);
        [gesture setTranslation:CGPointZero inView:self];
        CGPoint suffixPoint = [gesture translationInView:self];

//        XXLog(@"后gesture.x = %f, gesture.y = %f", suffixPoint.x, suffixPoint.y);
    ...
    if (gesture.state == UIGestureRecognizerStateEnded ||
        ...
        XXLog(@"横向速度：%.f  纵向速度： %.f",velocity.x,velocity.y);
        
//        XXLog(@"gesture.view.center.y = %f, _cardCenterY = %f", gesture.view.center.y , _cardCenterY);
        float ratio_w = (gesture.view.center.x - _cardCenterX) / _cardCenterX;
        float ratio_h = (gesture.view.center.y - _cardCenterY) / _cardCenterY;
//        XXLog(@"ratio_w = %f, ratio_h = %f", ratio_w, ratio_h);
        
        YSLDraggableDirection direction = YSLDraggableDirectionDefault;
        if (fabs(ratio_h) > fabs(ratio_w)) {
            
//            XXLog(@"_canDraggableDirection = %zd, YSLDraggableDirectionDown = %zd", _canDraggableDirection, YSLDraggableDirectionDown);
//            XXLog(@"_canDraggableDirection & YSLDraggableDirectionUp = %zd", _canDraggableDirection & YSLDraggableDirectionUp);
//            XXLog(@"kDragCompleteCoefficient_height_default = %f", kDragCompleteCoefficient_height_default);
//            XXLog(@"kDragCompleteCoefficient_height_default && (_canDraggableDirection & YSLDraggableDirectionDown) = %zd", kDragCompleteCoefficient_height_default && (_canDraggableDirection & YSLDraggableDirectionDown));
            if (ratio_h < - kDragCompleteCoefficient_height_default && (_canDraggableDirection & YSLDraggableDirectionUp)) {
                // up
                XXLog(@"_canDraggableDirection = %zd, YSLDraggableDirectionUp = %zd", _canDraggableDirection, YSLDraggableDirectionUp);
                XXLog(@"_canDraggableDirection & YSLDraggableDirectionUp = %zd", _canDraggableDirection & YSLDraggableDirectionUp);
                
                XXLog(@"ratio_h = %f, -kDragCompleteCoefficient_height_default = %f", ratio_h, -kDragCompleteCoefficient_height_default);
                direction = YSLDraggableDirectionUp;
            }
            XXLog(@"ratio_h = %f, 与 = %d", ratio_h, kDragCompleteCoefficient_height_default && (_canDraggableDirection & YSLDraggableDirectionDown));
